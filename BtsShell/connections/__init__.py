...
def execute_ssh_command_bare(command):return SSHCONNECTION.execute_ssh_command_bare(command)
def get_ssh_recv_content():return SSHCONNECTION.get_ssh_recv_content()
def ssh_put_file(src, des, mode='0744', newlines='LF'):return SSHCONNECTION.ssh_put_file(src, des, mode, newlines)
def ssh_get_file(src, des):return SSHCONNECTION.ssh_get_file(src, des)
...
def execute_tm500_command_without_check(command, length = '2048', delay_timer = '0.1', exp_prompt='', newtimeout = 600, ignore_output = 'N'):
    ...
    """
    return BTSTELNET.execute_tm500_command_without_check(command, length, delay_timer, exp_prompt, newtimeout)
...
def Check_Axis_power(RF_Mode,RF_Count = 1,RF1_ip = '192.168.254.129',RF2_ip = '192.168.254.137',RF3_ip = '192.168.254.145',username = 'root',password = 'axis',port = 23):
    ...
    """
    BTSTELNET.Check_Axis_power(RF_Mode,RF_Count = 1,RF1_ip = '192.168.254.129',RF2_ip = '192.168.254.137',RF3_ip = '192.168.254.145',username = 'root',password = 'axis',port = 23)
...
if __name__ == '__main__':
    ...
    # con = connect_to_host('192.170.10.253', '23', 'tdlte-tester', 'btstest')
    # con.debuglevel = 1
    # execute_shell_command_without_check("")
    #execute_shell_command('cd /d \"C:\\TestCases\\trunk\\RL15\\C&P\\Capacity\\bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb\"')
    #execute_shell_command('cd \"CP KPI_DLUL continuous throughput single cell-Multi UE fix PRB\"')
    #execute_shell_command('cd /d \"C:\\TestCases\\trunk\\RL15\\C&P\\Capacity\\CP_KPI_DLUL_continuous_throughput_single_cell-Multi_UE_fix_PRB"')
    pass
