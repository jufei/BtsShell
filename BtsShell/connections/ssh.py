...
class MySshLib(SSHLibrary):
    ...
    def open_connection(self, host, alias=None, port=22, timeout=None,
        ...
        return SSHLibrary.open_connection(self, host, alias, port, timeout, \
                                          newline, prompt)
...
class SshConnection:
    ...
    def ssh_put_file(self, src, des, mode='0744', newlines='LF'):return self._current.put_file(src, des, mode, newlines)
    def ssh_get_file(self, src, des='.'):return self._current.get_file(src, des)
    def execute_ssh_command_bare(self, command):return self._current.write_bare(command)
    def get_ssh_recv_content(self):return self._current.read()
...
if __name__ == '__main__':
    ...
    tl = SshConnection()
    print '1.login'
    tt = tl.connect_to_ssh_host('10.68.170.69','22','root','centos')
    #tl.connect_to_ssh_host('10.68.143.135', '22', 'Nemuadmin', 'nemuuser')
    print '2.ls'
    print tl.execute_ssh_command('ls -l')
    print '3.pwd'
    print tl.execute_ssh_command('pwd')
    print '4.logout', tt.host
    tl.disconnect_from_ssh()
    tl.disconnect_all_ssh()
