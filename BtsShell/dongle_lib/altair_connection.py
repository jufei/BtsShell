...
class Altair_control():
    def __init__(self, args):
        if not 'PATH' in args.keys():
            args['PATH'] = os.path.join(get_btsshell_path(), "dongle_lib", "altair_control.py")
        else:
            pass
        self.var_dict = args
...
self.var_dict['PATH'], self.var_dict['PORT'], self.var_dict['FREQUENCY'])
        ...
        if self.var_dict.has_key('IPVER'):
            cmd = cmd + " --ipver=%s" % (self.var_dict['IPVER'])
    ...
    def dongle_reboot(self, atcmd = "AT+CFUN=0"):
        """This keyword used for sending reset AT command to UE through COM port.

        Example
        | Dongle Detach |
        """
        if 'SWITCHPORT' in self.var_dict:
            cmd = "python \"%s\" -o reset --port=%s --switchport=%s" % (self.var_dict['PATH'], self.var_dict['PORT'], self.var_dict['SWITCHPORT'])
        else:
...
if __name__ == "__main__":
    ...
    obj = Altair_control({'PORT':'COM5', 'FREQUENCY':'38000'})
